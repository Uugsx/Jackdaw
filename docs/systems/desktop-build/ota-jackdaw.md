...
Runbook for **automatic updates** of the Jackdaw Electron desktop app via **GitHub Releases** (private repo `Uugsx/Jackdaw`).
...
| `JACKDAW_GH_UPDATE_TOKEN` | Fine-grained PAT: read private repo + releases; baked into installers |

CI **fails** if `gh-update-token.txt` is empty (`jackdaw-brand.sh`).
...
| File | Purpose |
|------|---------|
| `app-update.yml` | Generated by electron-builder — feed URL, owner, repo (private) |
| `gh-update-token.txt` | PAT for authenticated GitHub API / downloads (`extraResources` in `electron-builder.yml`) |
