...
`json-autotranslate` is a CLI tool that reads a directory of locales files and sends them to the translation service for translation, then saves the translated files back to the directory. Jackdaw uses the upstream [json-autotranslate](https://github.com/leolabs/json-autotranslate) package from npm.
