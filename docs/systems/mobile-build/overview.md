...
1. Go to `app/build`.
2. Run `./jackdaw-brand.sh`. Because there's no assets for `@capacitor/assets` in `mobile/` not even default assets for it to generate icons and splash screens and it will throw an error preventing the app from building. `@capacitor/assets` generates icons and splash screens from `mobile/assets`.
3. Go to `mobile/`.
