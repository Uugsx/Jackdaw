...
export class IncomingInvitation {
  ...
  calEvent(): Event | null {
    ...
    if (this.event.recurrenceStartTime) {
      let occurrence = event?.getOccurrenceByDate(this.event.recurrenceStartTime, false);
      if (occurrence) {
        return occurrence;
      }
      // Invite only to a single exception: match stored exception directly.
      let start = this.event.recurrenceStartTime.getTime();
      return this.calendar.events.find(ev =>
        ev.calUID == this.event.calUID &&
        ev.recurrenceStartTime &&
        ev.recurrenceStartTime.getTime() == start) ?? null;
    }
    return event ?? null;
  }
