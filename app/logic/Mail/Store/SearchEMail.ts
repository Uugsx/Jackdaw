...
export class SearchEMail extends Observable {
  ...
  get folder(): Folder | null {
    return this._folder ?? (this.folderID && (this._folder ??= this.account?.findFolder(folder => folder.id == this.folderID))) ?? null;
  }
  ...
  fromJSON(json: any) {
    ...
    this.isImportant = boolean(json.isImportant);
  ...
  matches(email: EMail): boolean {
    ...
    if (this.bodyText) {
      let needle = this.bodyText.toLowerCase();
      let haySubject = email.subject?.toLowerCase() ?? "";
      // Prefer `.text` so HTML-only bodies (converted on read) are searchable
      let hayBody = (email.text || email.rawText || "").toLowerCase();
      if (!haySubject.includes(needle) && !hayBody.includes(needle)) {
        return false;
      }
    }
