...
export class IMAPAccount extends MailAccount {
   ...
   * @see this.namespaces
   */
  async getNamespaces(): Promise<Record<IMAPNamespace, IMAPNamespaceRecord[]>> {
    let conn = imapInternals(await this.connection());
    // This ImapFlow command checks for the NAMESPACE capability
    // and returns the default namespaces if the server doesn't support it.
    // It caches the result on the connection.
    return await conn.run("NAMESPACE") ?? conn.namespaces;
  }
 ...
 */
export type IMAPNamespace = "personal" | "other" | "shared";
/**
 ...
 */
export interface IMAPNamespaceRecord { prefix: string; delimiter: string };

/** ImapFlow members that exist at runtime, but are missing from its typings. */
export interface ImapFlowInternals {
  /** Sends a raw IMAP command. Resolves once the tagged response arrives.
   * `next()` must be called to let ImapFlow continue processing. */
  exec(command: string, attributes?: any[], options?: any): Promise<{ response: any, next: () => void }>;
  /** Runs one of ImapFlow's built-in command handlers, e.g. "NAMESPACE". */
  run(command: string, ...args: any[]): Promise<any>;
  namespaces?: Record<IMAPNamespace, IMAPNamespaceRecord[]>;
}

export function imapInternals(conn: ImapFlow): ImapFlow & ImapFlowInternals {
  return conn as ImapFlow & ImapFlowInternals;
}
