...
export class ExchangePermission {
  ...
  toOWACalendarPermission() {
    return Object.assign({
      ...
      ? { DistinguishedUser: this.distinguishedUser, }
      : { PrimarySmtpAddress: this.emailAddress, },
      // `CalendarPermissionType` requires `CalendarPermissionLevel`;
      // `PermissionLevel` belongs to the non-calendar `PermissionType`.
    }, { CalendarPermissionLevel: "Custom" }, this.exchangePermissions);
  }
}
