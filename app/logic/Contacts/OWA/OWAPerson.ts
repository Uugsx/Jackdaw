...
export class OWAPerson extends ExchangePerson {
  ...
  fromJSON(json: any): OWAPerson {
    ...
    this.emailAddresses.replaceAll(extractSMTPAddresses(json));
    ...
    this.streetAddresses.replaceAll(Object.keys(PhysicalAddressPurposes)
      .flatMap(purpose => {
        let entries = json[purpose + "AddressesArray"]
          ?.filter((entry: any) => entry?.Value)
          .map((entry: any) =>
            new ContactEntry(OWAPerson.owaToStreetAddress(entry.Value),
              PhysicalAddressPurposes[purpose])) || [];
        // Fallback: Persona*Addresses may store multiple $#$ blocks joined by RS
        if (!entries.length) {
          let fieldKey = "Persona" + purpose + "Addresses";
          let packed = json[fieldKey] ?? json[purpose + "Addresses"];
          if (typeof packed == "string" && packed && packed != "$#$$#$$#$$#$") {
            entries = packed.split("\u001E").filter(Boolean).map(block =>
              new ContactEntry(OWAPerson.owaEncodedToStreet(block),
                PhysicalAddressPurposes[purpose]));
          }
        }
        return entries;
  ...
  /** Decode a single $#$ Persona address block back to a street string. */
  protected static owaEncodedToStreet(encoded: string): string {
    let parts = encoded.split("$#$");
    let address = new StreetAddress();
    let keys = ["street", "city", "state", "postalCode", "country"] as const;
    for (let i = 0; i < keys.length; i++) {
      address[keys[i]] = sanitize.nonemptystring(parts[i], "");
    }
    return address.toString();
  }
  ...
  protected toFields(): Record<string, string> {
    ...
    // Exchange Contact PhysicalAddresses has one slot per purpose (Business/Home/Other).
    // Persona AddressesArray can list several; encode all of each purpose so save
    // does not silently drop extras (joined $#$ blocks). First block stays the
    // primary address Exchange uses for that purpose.
    fields.PersonaBusinessAddresses = OWAPerson.streetAddressesToOWA(
      this.streetAddresses.contents.filter(entry => entry.purpose == "work").map(e => e.value));
    fields.PersonaHomeAddresses = OWAPerson.streetAddressesToOWA(
      this.streetAddresses.contents.filter(entry => entry.purpose == "home").map(e => e.value));
    fields.PersonaOtherAddresses = OWAPerson.streetAddressesToOWA(
      this.streetAddresses.contents.filter(entry => entry.purpose == "other").map(e => e.value));
    fields.PersonaBodies = this.notes;
    // console.log("OWAPerson save()", fields);
    return fields;
  }

  /** Encode one or more street addresses for Persona*Addresses PropertyUpdates. */
  protected static streetAddressesToOWA(streets: string[]): string {
    let nonempty = streets.filter(Boolean);
    if (!nonempty.length) {
      return "$#$$#$$#$$#$";
    }
    return nonempty.map(s => OWAPerson.streetAddressToOWA(s)).join("\u001E");
  }
...
/** Prefer SMTP addresses from the EmailAddresses array; fall back to singular EmailAddress. */
function extractSMTPAddresses(json: any): ContactEntry[] {
  let fromArray = (json.EmailAddresses ?? [])
    .filter((address: any) => !address.RoutingType || address.RoutingType == "SMTP")
    .map((address: any) =>
      new ContactEntry(sanitize.emailAddress(address.EmailAddress, null), "work", "mailto")
    )
    .filter((ce: ContactEntry) => ce.value);
  if (fromArray.length) {
    return fromArray;
  }
  let singular = json.EmailAddress;
  if (singular?.EmailAddress && (!singular.RoutingType || singular.RoutingType == "SMTP")) {
    let value = sanitize.emailAddress(singular.EmailAddress, null);
    if (value) {
      return [new ContactEntry(value, "work", "mailto")];
    }
  }
  return [];
}
