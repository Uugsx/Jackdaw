...
export class OWAAddressbook extends ExchangeAddressbook {
  ...
  getPersonByItemID(id: string): OWAPerson | undefined {
  ...
  getGroupByItemID(id: string): OWAGroup | undefined {
    return this.groups.find(p => p.itemID == id);
  }

  /**
   * Item-level RowNotification on the contacts folder: refresh one contact/DL
   * when possible; fall back to a full FindPeople pass for new items.
   */
  async refreshFromRowNotification(itemID: string | null, eventType: string) {
    if (eventType == "RowDeleted" && itemID) {
      let person = this.getPersonByItemID(itemID);
      if (person) {
        this.persons.remove(person);
        await person.deleteLocally();
        return;
      }
      let group = this.getGroupByItemID(itemID);
      if (group) {
        this.groups.remove(group);
        await group.deleteLocally();
        return;
    ...
    if (itemID) {
      ...
      if (person?.personaID) {
        try {
          let response = await this.callOWA(owaGetPersonaRequest(person.personaID));
          person.fromJSON(Object.assign({}, response.Persona, {
            PersonaId: response.Persona?.PersonaId ?? { Id: person.personaID },
            Notes: person.notes,
          }));
          let responseNotes = await this.callOWA(new OWAGetNotesForPersonaRequest(person.personaID));
          person.notes = sanitize.nonemptystring(
            responseNotes.PersonaWithNotes?.BodiesArray?.[0]?.Value?.Value, person.notes);
          await person.saveLocally();
          return;
        } catch (ex) {
          this.account.errorCallback(ex);
      ...
      if (group?.personaID) {
        try {
          await this.listGroups([{
            PersonaId: { Id: group.personaID },
            EmailAddress: { ItemId: { Id: itemID } },
            PersonaTypeString: "DistributionList",
          }]);
          return;
        } catch (ex) {
          this.account.errorCallback(ex);
