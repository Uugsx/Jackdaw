...
<script lang="ts">
  ...
  import { getOAuth2BuiltIn, getOAuth2BuiltInError } from "../../../logic/Auth/OAuth2Util";
  ...
  async function onContinue() {
    if (step == Step.EmailAddress) {
      ...
      if (config.setup?.instructions) {
        ...
        assert(config.oAuth2, getOAuth2BuiltInError(config) ?? gt`Could not find OAuth2 config for ${config.hostname}`);
        step = Step.Login;
      } else {
        step = Step.CheckConfig;
      }
