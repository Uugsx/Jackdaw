<hbox class="invitation buttons">
  ...
  {:else if myParticipation == InvitationResponse.Tentative}
    <Button
      label={$t`Maybe *=> Maybe attend the meeting`}
      icon={MaybeIcon}
      selected={true}
      onClick={onOpenInCalendar}
      classes="maybe done font-normal" />
    <ButtonMenu buttonIcon={ChevronDownIcon}>
      <MenuItem
        label={$t`Change to Accept *=> attend the meeting`}
        onClick={onAccept}
...
<script lang="ts">
  ...
  async function respond(response: InvitationResponseInMessage) {
    ...
    try {
      ...
      myParticipation = previous;
      throw ex;
    }
  }

  async function onAccept() {
    await catchErrors(() => respond(InvitationResponse.Accept));
  }
  async function onTentative() {
    await catchErrors(() => respond(InvitationResponse.Tentative));
  }
  async function onDecline() {
    await catchErrors(() => respond(InvitationResponse.Decline));
  }
...
<style>
  ...
  .buttons :global(button.maybe.done) {
    color: white;
