...
<script lang="ts">
  ...
  import { CalendarEventJackdawApp, calendarApp } from "../CalendarJackdawApp";
  ...
  $: isFullWindow = $selectedApp instanceof CalendarEventJackdawApp;
  let isDeleteSeriesOpen = false;

  function onEdit() {
    calendarApp.showEvent(event); // open full screen
    event.startEditing();
  }
  ...
  function onClose() {
    ...
    let me = calendarApp.subApps.find(app => app instanceof CalendarEventJackdawApp && app.windowParams.event == event);
    calendarApp.subApps.remove(me);
    if (isFullWindow) {
      goBack();
    } else {
