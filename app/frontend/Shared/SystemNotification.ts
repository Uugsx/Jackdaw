...
export class SystemNotification {
  ...
  title: string;
  /** Second line, between `title` and `body`. macOS only; on the other
   * platforms `show()` folds it into the first line of the body. */
  ...
  protected get bodyWithSubtitle(): string {
  ...
  protected async showOSPopup() {
    let hasSubtitleLine = getOSName() == "macintosh";
    let popup = await appGlobal.remoteApp.newOSNotification({
      title: this.title,
      subtitle: hasSubtitleLine ? this.subtitle ?? undefined : undefined,
      body: hasSubtitleLine ? this.body : this.bodyWithSubtitle,
      // icon: url,
    ...
    if (this.onReply) {
      popup.on("reply", async (event, replyText) => {
        onClose();
        let text = typeof replyText == "string" ? replyText : "";
        if (text.trim()) {
